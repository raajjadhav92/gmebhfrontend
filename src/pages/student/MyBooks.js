...
const MyBooks = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await api.get(`/api/books/student/${user.studentId}`);
        
        if (response.data.success) {
          const { transactions } = response.data.data;
          const allTransactions = transactions || [];

          // 1. Identify currently issued books from transactions
          const issuedTransactions = allTransactions.filter(t => t.status === 'issued');

          // 2. De-duplicate the issued books to get a unique list of current books
          const uniqueIssuedBooks = Array.from(new Map(issuedTransactions.map(t => [t.book._id, t.book])).values());
          setCurrentBooks(uniqueIssuedBooks);

          // 3. Create a history of completed (returned or lost) transactions
          const historicalTransactions = allTransactions.filter(
            t => t.status === 'returned' || t.status === 'lost'
          );
          setHistoryBooks(historicalTransactions);
        }
      } catch (err) {
                      ...
                      <div className={`p-4 ${isOverdue ? 'bg-red-50' : 'bg-white'}`}>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <p className="font-medium">{book.dateOfIssue ? new Date(book.dateOfIssue).toLocaleDateString() : 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-gray-500">Due Date</p>
                            <p className={`font-medium ${isOverdue ? 'text-red-600' : 'text-gray-900'}`}>
                              {book.expectedReturnDate ? new Date(book.expectedReturnDate).toLocaleDateString() : 'N/A'}
                            </p>
                          </div>
                        </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Returned On</th>
                    ...
                    {historyBooks.map((transaction) => {
                      ...
                      return (
                        <tr key={transaction._id}>
                          <td className="px-6 py-4">
                            <div className="flex items-center">
                              <div>
                                ...
                                <div className="text-sm text-gray-400">ID: {transaction.book.bookId}</div>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {transaction.actualReturnDate ? new Date(transaction.actualReturnDate).toLocaleDateString() : 'Not Returned'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {transaction.fine > 0 ? `₹${transaction.fine}` : 'No Fine'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              transaction.status === 'returned' ? 'bg-green-100 text-green-800' :
                              transaction.status === 'lost' ? 'bg-red-100 text-red-800' :
